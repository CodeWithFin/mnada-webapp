          ...
          <div className="newsletter_form-block">
            <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-0 max-w-md mx-auto relative border border-[#e5e5e5]">
              <input 
                ...
                className="flex-1 bg-white h-14 md:h-12 px-6 text-sm md:text-xs outline-none text-[#1c1a19] placeholder:text-gray-400 placeholder:tracking-widest placeholder:uppercase font-light"
                disabled={isSubmitting}
              />
              <button 
                type="submit" 
                disabled={isSubmitting}
                className="bg-[#1c1a19] text-white h-14 md:h-12 px-8 font-bold uppercase text-[10px] tracking-widest hover:bg-[#a58c69] transition-colors disabled:opacity-50"
              >
                {isSubmitting ? <Icon icon="lucide:loader" className="animate-spin" width="18" /> : 'Subscribe'}
              </button>
            </form>
